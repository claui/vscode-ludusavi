...
This is the source code repository for the `ludusavi`
VS Code extension.
...
## Updating the upstream schemas

From the project root directory, run the following command line:

```shell
curl -LO --output-dir extension/vendor/ \
  "https://raw.githubusercontent.com/mtkennerly/ludusavi-manifest/master/data/schema{,.strict}.yaml"
```
...
4. Run the `yarn […] ovsx publish` command with the correct
   ...
   ```bash
   yarn workspace extension ovsx publish "dist/ludusavi-$(jq -r .version extension/share/dist/package.json).vsix"
   ```
...
vscode-ludusavi depends only indirectly, e.g. through a
...
all the way down to vscode-ludusavi), the patch may be
...
|---|---|---|---|
| `.` | This directory | MIT | [License](./LICENSE) |
...
| `./extension` | Source code for this VS Code extension | MIT | [License](./extension/LICENSE.txt) |
